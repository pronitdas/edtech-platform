  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4",
        messages: [
          { role: "system", content: "You are a content generator for educational material." },
...
serve(async (req) => {
  try {
    ...
    if (chapterError) throw new Error(`Error inserting chapter: ${chapterError.message}`);
    ...
    if (edTechError) throw new Error(`Error inserting EdTech content: ${edTechError.message}`);
    ...
    return new Response(
      JSON.stringify({ message: "Chapter, content, and quiz seeded successfully!" }),
