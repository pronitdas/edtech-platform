...
export function calculateUnderstandingLevel(
  ...
  const weightedScore =
    quizScore * quizWeight + engagementScore * engagementWeight
...
export function generateRecommendations(
  ...
  if (videoStats) {
    ...
    if (completionRate < 0.5) {
      recommendations.push(
        'Try to watch more videos completely to improve your understanding.'
      )
    }
    ...
    if (watchPercentage < 0.7) {
      recommendations.push(
        'Consider watching videos more thoroughly to get the full benefit.'
      )
    }
  }
  ...
  if (quizStats) {
    const quizCompletionRate =
      quizStats.quizzes_completed / (quizStats.total_quizzes || 1)
    if (quizCompletionRate < 0.7) {
      recommendations.push(
        'Complete more quizzes to test your knowledge and identify areas for improvement.'
      )
    }
    ...
    if (averageScore < 0.6) {
      recommendations.push(
        'Review the content and retake quizzes to improve your score.'
      )
    }
  }
  ...
  if (interactionSummary) {
    ...
    if (totalInteractions < 10) {
      recommendations.push(
        'Engage more with the learning materials to improve retention.'
      )
    }
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push(
      'Keep up the good work! Continue with your current learning approach.'
    )
  }
