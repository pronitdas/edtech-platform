...
const CoreGraphCanvas: React.FC<CoreGraphCanvasProps> = ({
    ...
    const throttledMouseMove = useCallback(
        throttle((p: p5, checkInteractions: (x: number, y: number) => HoveredElement | null) => {
    ...
    useEffect(() => {
        ...
        const sketch = (p: p5) => {
            ...
            let touchSensitivity = drawingStrategy.getTouchSensitivity?.() || 20;
            ...
            const handleTouchZoom = (event: TouchEvent): void => {
                if (event.touches.length === 2) {
                    ...
                    if (lastTouchDistance > 0) {
                        ...
                        const centerPointBeforeZoom = mapCanvasToPoint({ x: centerX, y: centerY });
                        const centerPointAfterZoom = {
                            x: (centerX - offset.x) / newZoom,
                            y: (centerY - offset.y) / newZoom
                        };

                        const newOffsetX = offset.x - (centerPointAfterZoom.x - centerPointBeforeZoom.x) * newZoom;
                        const newOffsetY = offset.y - (centerPointAfterZoom.y - centerPointBeforeZoom.y) * newZoom;
            ...
            const checkInteractions = (x: number, y: number): HoveredElement | null => {
            ...
            p.setup = () => {
                ...
                canvasElement.addEventListener('wheel', (e: WheelEvent) => e.preventDefault());
