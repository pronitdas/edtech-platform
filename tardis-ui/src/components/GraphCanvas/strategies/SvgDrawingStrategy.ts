...
export class BasicSvgDrawingStrategy implements SvgDrawingStrategy {
    ...
    private transformPoint(point: Point, zoom: number = 1, offset: Offset = { x: 0, y: 0 }): Point {
        return {
        ...
        return {
            ...
            transformedShapes: data.shapes.map(shape => ({
                ...
                topLeft: shape.topLeft ?
                    this.transformPoint(shape.topLeft, zoom, offset) : undefined,
                bottomRight: shape.bottomRight ?
                    this.transformPoint(shape.bottomRight, zoom, offset) : undefined,
                center: shape.center ?
                    this.transformPoint(shape.center, zoom, offset) : undefined
            })),
