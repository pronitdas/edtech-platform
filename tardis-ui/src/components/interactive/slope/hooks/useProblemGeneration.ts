...
import { LearningContext, ProblemGenerationConfig, ProblemGenerationStats, ProblemDifficulty, SolutionResult } from '../types'; // Import types from index.ts
...
export function useProblemGeneration({
...
}: ProblemGenerationConfig & { learningContext?: LearningContext, setStats: React.Dispatch<React.SetStateAction<ProblemGenerationStats>>, setProblemHistory: React.Dispatch<React.SetStateAction<any[]>> }) { // Update type definition and add setStats and setProblemHistory
  // State
  ...
  const generateProblem = useCallback(() => {
    ...
    let problemType: 'slope' | 'undefined-slope' | 'zero-slope' | 'fractional-coordinates' = 'slope'; // Default to general slope problem
    ...
    if (recentHistory && recentHistory.length > 0) {
      ...
      if (undefinedSlopeAttempts > 0 && undefinedSlopeAttempts >= incorrectAttempts.length * 0.5) {
        problemType = 'undefined-slope';
      } else if (zeroSlopeAttempts > 0 && zeroSlopeAttempts >= incorrectAttempts.length * 0.5) {
        problemType = 'zero-slope';
      } else if (fractionalCoordinateAttempts > 0 && fractionalCoordinateAttempts >= incorrectAttempts.length * 0.5) {
    ...
    const generatePoints = (diff: ProblemDifficulty, type: typeof problemType, context?: LearningContext) => {
      let range, offset;
      ...
      if (context && context.stats.difficultyStats[diff].attempted >= minAttemptsForAdaptation) {
      ...
      const generateCoordinate = (currentRange: number, currentOffset: number, fractions: boolean) => {
        if (fractions && Math.random() > 0.5) { // 50% chance of fractional coordinate if allowed
          const integerPart = Math.floor(Math.random() * currentRange + currentOffset);
      ...
      switch (type) {
        case 'undefined-slope':
          ...
          y2 = y1;
          while (Math.abs(y2 - y1) < 0.001) { // Ensure y1 and y2 are distinct
        ...
        case 'zero-slope':
          ...
          x2 = x1;
          while (Math.abs(x2 - x1) < 0.001) { // Ensure x1 and x2 are distinct
        ...
        case 'fractional-coordinates':
          ...
          } while (Number.isInteger(x1) && Number.isInteger(y1) && Number.isInteger(x2) && Number.isInteger(y2) || Math.abs(x2 - x1) < 0.001); // Ensure distinct x values
    ...
    const { x1, y1, x2, y2 } = generatePoints(difficulty, problemType, learningContext);
...
Step 4: The equation of the line is ${equationText}`;
  ...
  const checkSolution = useCallback((lineData: { slope: number | null; point1: any; point2: any }) => {
    ...
    if (currentProblem.expectedSlope === null) {
      ...
      // Update stats
      setStats(prev => { // Use setStats from provider
        // Add to history, keeping only maxHistoryLength records
        const newHistory: SolutionResult[] = [...prev.history, { problemId: currentProblem?.id || '', isCorrect: isAnswerCorrect, timeSpent: 0, difficulty: currentProblem?.difficulty || 'easy' }];
    ...
    return isAnswerCorrect;
  }, [currentProblem, maxHistoryLength, setStats, setProblemHistory]); // Add setStats and setProblemHistory to dependencies
  ...
  useEffect(() => {
    const adaptDifficulty = () => {
      ...
      const currentDifficultyStats = learningContext.stats.difficultyStats[difficulty];
      ...
      const successRate = currentDifficultyStats.correct / currentDifficultyStats.attempted;
      ...
      if (successRate > increaseThreshold && difficulty !== 'hard') {
        ...
        console.log(`Adapting difficulty: Increasing from ${difficulty} to ${newDifficulty}`);
        ...
        console.log(`Adapting difficulty: Decreasing from ${difficulty} to ${newDifficulty}`);
        changeDifficulty(newDifficulty);
      }
    };
    ...
    const currentDifficultyStats = learningContext?.stats.difficultyStats[difficulty];
    if (learningContext && currentDifficultyStats && currentDifficultyStats.attempted > 0 && currentDifficultyStats.attempted % 3 === 0) {
      adaptDifficulty();
    }
  ...
  }, [problems.length, currentProblemId, generateProblem, setCurrentProblemId, problems]); // Add problems and setCurrentProblemId to dependencies
