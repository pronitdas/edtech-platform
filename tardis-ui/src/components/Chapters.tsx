  ...
  <div className={`${compact ? 'mb-2' : 'mb-4'}`}>
    <div className='relative'>
      <input
        ...
        className={`w-full ${compact ? 'px-3 py-1.5 text-sm' : 'px-4 py-2'} rounded-md border border-gray-600 bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500`}
      />
...
const ChapterCard = ({ chapter, chapterMeta, onClick }: { chapter: Chapter; chapterMeta: any; onClick: () => void }) => (
  <Card
    ...
    onClick={onClick}
  >
    <CardHeader className='rounded-t-lg bg-gradient-to-r from-blue-500 to-purple-600 p-3 text-white'>
...
const ChapterMetaBadges = ({ chapterMeta }: { chapterMeta: any }) => (
  <div className='flex flex-wrap gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-red-500 p-2'>
    ...
    <MetaBadge available={chapterMeta?.has_summary} label='Summary' />
  </div>
)
...
const MetaBadge = ({ available, label }: { available: boolean; label: string }) => (
  <div className='flex items-center space-x-1'>
    {available ? (
      ...
      <>
        <PlusCircleIcon className='h-4 w-4 text-blue-300' />
...
const ChapterList = ({ chapters, chaptersMeta, onLessonClick }: { chapters: Chapter[]; chaptersMeta: any[]; onLessonClick: (chapter: Chapter) => void }) => (
  <div className='mt-2 space-y-2'>
    {chapters.length > 0 ? (
      ...
      <div className='py-4 text-center text-gray-400'>No chapters found</div>
    )}
  </div>
)
