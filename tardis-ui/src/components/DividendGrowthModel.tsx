...
const DividendGrowthCalculator = () => {
      ...
      const sketch = (p) => {
        ...
        const drawGraph = (p) => {
          ...
          for (let year = 0; year <= years; year++) {
            const currentDividend = values.dividend * Math.pow(1 + growthRate, year);
          ...
          points.forEach(point => {
            ...
            const y = p.map(point.y, 0, maxDividend * 1.2, canvasH - margin, margin);
            p.vertex(x, y);
            p.push();
            p.fill(0, 120, 255);
            p.ellipse(x, y, 6, 6);
          ...
          for (let i = 0; i <= yLabels; i++) {
            ...
            const y = p.map(value, 0, maxDividend * 1.2, canvasH - margin, margin);
            p.textAlign(p.RIGHT);
  ...
  return (
    <Card className="w-full overflow-y-scroll max-w-4xl bg-neutral-900 text-neutral-100">
      <CardHeader>
        <CardTitle>Interactive Dividend Growth Model</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="mb-4">
          The Dividend Growth Model (Gordon Growth Model) is used to calculate the intrinsic value of a stock based on
          future dividend payments that grow at a constant rate.
        </p>

        <div className="bg-neutral-800 p-4 rounded-md mb-6">
          <p className="text-lg mb-2">Formula: P = D₁ / (r - g)</p>
        ...
        <div className="flex flex-wrap gap-6">
          ...
          <div className="flex-1 min-w-[400px]" ref={canvasRef} />
        </div>
      </CardContent>
    </Card>
  );
