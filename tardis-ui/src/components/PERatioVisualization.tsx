...
const PERatioVisualization = () => {
  ...
  useEffect(() => {
    ...
    if (companiesRef.current.length === 0) {
      companiesRef.current = Array.from(
        { length: 20 },
        () => new Company(width, height)
      )
    }
    ...
    const draw = (): void => {
      ...
      ctx.fillText(
        `Market Average PE: ${(totalPE / companiesRef.current.length).toFixed(1)}`,
        20,
        30
      )
  ...
  return (
    <Card className='w-full overflow-y-auto bg-slate-800 text-white'>
      <CardHeader>
        <CardTitle className='text-xl'>
          P/E Ratio & Market Risk Relationship
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className='space-y-6'>
          <div className='rounded-lg bg-black/30 p-4'>
            <p>
              This visualization demonstrates how Price-to-Earnings (P/E) ratios
              respond to market risk levels. Each circle represents a company.
            </p>
          </div>
          ...
          <div className='relative h-[300px]'>
            ...
            {hoveredCompany && (
              <div
                className='absolute rounded bg-black/80 p-2 text-sm'
                style={{
          ...
          <div className='space-y-4'>
            ...
            <div className='flex items-center space-x-2'>
              <div className='h-5 w-5 rounded-full bg-white/70'></div>
              <span>Circle Size = P/E Ratio (larger = higher P/E)</span>
            </div>
            <div className='flex items-center space-x-2'>
              <div className='h-5 w-5 rounded-full bg-[rgba(255,200,100,0.7)]'></div>
          ...
          <div className='space-y-2'>
            ...
            <p>
              → Lower Risk: Investors are confident in earnings, willing to pay
              higher prices (higher P/E)
            </p>
            <p>
              → Higher Risk: Investors demand lower prices relative to earnings
              (lower P/E)
            </p>
            <p>
              → Circle Pulsing: Represents market uncertainty and price
              fluctuations
            </p>
          </div>
        </div>
