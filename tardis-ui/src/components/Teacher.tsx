...
const auth = { type: "key" as const, clientKey: "Z29vZ2xlLW9hdXRoMnwxMDEyNTM3MTA4MTkyOTU5NDEyNjA6WGdmNkdPZUJYalo0dlU4Yko0UFU0" }; // Use "key" as a constant literal type
...
const Teacher = () => {
    ...
    useEffect(() => {
        const initializeAgent = async () => {
            ...
            try {
                const manager = await sdk.createAgentManager(teacherId, { auth, callbacks, streamOptions });
    ...
    return (
        <div className="text-white absolute bottom-0 right-0">
            ...
            <button
                onClick={speakText}
                className={`${connectionState == "connected" ? 'bg-blue-500' : 'bg-red-500'}  w-48  hover:bg-blue-700 text-white font-bold py-2 rounded`}
