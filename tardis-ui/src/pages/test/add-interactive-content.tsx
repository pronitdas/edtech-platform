...
const AddInteractiveContent = () => {
  ...
  const handleAddInteractiveContent = async () => {
    ...
    try {
      ...
      setStatus('success')
      setMessage(
        `Interactive content added to chapter ${chapterId}. ${data?.length} rows updated.`
      )
    } catch (error) {
  ...
  return (
    <div className='mx-auto max-w-md p-8'>
      <h1 className='mb-6 text-2xl font-bold'>Add Interactive Content</h1>

      <div className='mb-4'>
        <label className='mb-1 block text-sm font-medium'>Chapter ID:</label>
        <input
          type='number'
          value={chapterId}
      ...
      <div className='mt-8'>
        ...
        <ol className='list-decimal space-y-2 pl-5'>
          <li>
            Enter the chapter ID where you want to add the interactive content
          </li>
          <li>Click the "Add Slope Drawing Tool" button</li>
          <li>Once successful, navigate to the chapter in the course</li>
          <li>
            You should now see a "Practice" tab with the slope drawing tool
          </li>
        </ol>
      </div>
    </div>
