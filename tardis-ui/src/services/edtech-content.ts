...
export const getEdTechContent = async (chapter, language = 'English') => {
  ...
  console.log(
    `EdTechContent_${language} from DB:`,
    EdTechContent,
    'Error:',
    error
  )
  ...
  if (!EdTechContent || EdTechContent.length === 0) {
    ...
    console.log(
      'Inserted new minimal EdTechContent:',
      newEdtechContent,
      'Error:',
      insertError
    )
...
export const getChapterMetaDataByLanguage = async (id: number, language) => {
  const { data: goldenContent, error: errTemp } = await supabase.rpc(
...
export const getChapters = async (id: number, language) => {
  ...
  if (chapters?.length == 0) {
    const { data } = await supabase
      ...
      .eq('id', id)
    data[0].docling_content.markdown = cleanMarkdown(
      data[0].docling_content.markdown
    )

    const { data: updatedData } = await insertChapters(
      id,
      data[0].docling_content
    )
    return updatedData
  }
...
export async function insertKnowledge(name, user, fileName) {
  ...
  const { data, error } = await supabase
    .from('knowledge')
    .insert([{ name, userId: user.id, filename: [fileName] }])
    .select()
  if (error) {
...
export async function uploadFiles(files, knowledge_id, fileType) {
  ...
  const promises = files.map(async file => {
    ...
    const { data, error } = await supabase.storage
      .from('media')
      .upload(basePath + file.name, blob)
  ...
  if (fileType === 'doc' || fileType === 'pdf') {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `Processing request for knowledge_id ${knowledge_id} returned status ${response.status}`
        )
        // We don't throw here to avoid failing the upload if processing fails
      } else {
        console.log(
          `Successfully triggered processing for knowledge_id ${knowledge_id}`
        )
      }
    } catch (error) {
      console.warn(
        `Failed to trigger processing for knowledge_id ${knowledge_id}:`,
        error
      )
      // We don't throw here to avoid failing the upload if processing fails
    }
...
async function uploadImages(files, knowledge_id) {
  ...
  const promises = files.map(async f => {
    ...
    const { data, error } = await supabase.storage
      .from('media')
      .upload(basePath + f.filename, blob)
  })
...
const insertChapters = async (knowledge_id, markdownData) => {
  ...
  const topics = distributeChaptersEqually(
    markdownData.markdown,
    structuredContent,
    lineRanges,
    'topics'
  )
  const subtopics = distributeChaptersEqually(
    markdownData.markdown,
    structuredContent,
    lineRanges,
    'subtopics'
  ...
  const chapters = distributeChaptersEqually(
    markdownData.markdown,
    structuredContent,
    lineRanges,
    'chapters'
...
export const insertKnowledgeDep = async markdownData => {
  ...
  const { data, error } = await supabase
    .from('knowledge')
    .insert([{ name: markdownData.filename, docling_content: markdownData }])
    .select()
...
export const getKnowledge = async () => {
  try {
    const { data, error } = await supabase
      .from('knowledge')
      .select(
        'id, name, seeded, video_url, status, filename, roleplay, difficulty_level, target_audience, prerequisites, summary, chapters_v1(k_id, subtopic, chaptertitle)'
      ) // Combined both sets of fields
...
export const mergeChaptersIntoLarger = (content, metadata, lineRanges, key) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const chapterText = contentLines
...
export const distributeChaptersEqually = (
  content,
  metadata,
  lineRanges,
  key
...
export class ContentService {
  ...
  async updateChapter(
    ...
    return apiClient.put<Chapter>(
      `/chapters/${knowledgeId}/${chapterId}`,
      updates
    )
  }
