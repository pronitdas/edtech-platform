...
export const useQuizAnalytics = (params: QuizAnalyticsParams) => {
  ...
  const trackStart = useCallback(
    (questionCount: number, difficulty?: string, timeLimit?: number) => {
  ...
  const trackSubmit = useCallback(
    (
      ...
      const correctAnswers = questionAnswersRef.current.filter(
        a => a.isCorrect
      ).length

      // Determine if passed (if passing score is provided)
      const passed =
        passingScore !== undefined ? score >= passingScore : undefined
