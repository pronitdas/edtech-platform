...
export function useCognitiveLoad(idleThresholdSeconds: number = 60) {
  ...
  const debouncedActivityHandler = useCallback(debounce((event: Event) => {
    const now = Date.now();
    setCognitiveState(prev => ({
      ...prev,
      lastActivityTimestamp: now,
    }));
  }, 100), []); // Debounce by 100ms, memoize the debounced function
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      setCognitiveState(prev => {
        const { errorCount, hesitationSeconds, idleTimeSeconds, recentKeydownTimestamps, recentClickTimestamps } = prev;
        ...
        const now = Date.now();
        const interactionWindow = 10000; // 10 seconds in milliseconds
        const keydownSpeed = recentKeydownTimestamps.filter(timestamp => now - timestamp < interactionWindow).length / (interactionWindow / 1000);
        const clickSpeed = recentClickTimestamps.filter(timestamp => now - timestamp < interactionWindow).length / (interactionWindow / 1000);
        ...
        if (errorCount > 6 || hesitationSeconds > 90 || idleTimeSeconds > 75) {
          newLoadLevel = 'high';
        } else if (errorCount > 3 || hesitationSeconds > 45 || idleTimeSeconds > 45) {
          newLoadLevel = 'medium';
        }
    ...
    }, [cognitiveState.errorCount, cognitiveState.hesitationSeconds, cognitiveState.idleTimeSeconds, cognitiveState.recentKeydownTimestamps.length, cognitiveState.recentClickTimestamps.length]); // Depend on metrics that influence load level
