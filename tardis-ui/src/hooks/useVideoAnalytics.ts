...
export const useVideoAnalytics = (params: VideoAnalyticsParams) => {
   ...
   */
  const trackPlay = useCallback((currentTime: number, totalDuration: number, quality?: string) => {
   ...
   */
  const trackPause = useCallback((currentTime: number, totalDuration: number, pauseReason?: string) => {
   ...
   * @param totalDuration Total video duration in seconds
   */
  const checkAndTrackProgress = useCallback((currentTime: number, totalDuration: number) => {
   ...
   * @param totalDuration Total video duration in seconds
   */
  const trackComplete = useCallback((totalDuration: number) => {
    // Prepare event data
    const eventData: VideoCompleteEvent = {
      ...
      completePercent: totalDuration > 0 
