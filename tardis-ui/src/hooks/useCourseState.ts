...
const getAvailableTabs = (content: any, chapter: any): TabConfig[] => {
  console.log("Getting available tabs with content:", content, "chapter:", chapter);
  
  // Use the existing BASE_TABS array defined above that has the correct structure
  // Filter tabs based on content availability
  const availableTabs = BASE_TABS.filter(tab => isTabAvailable(tab.key, content));
...
export const useCourseState = (initialContent: ChapterContent | null | undefined, chapter: ChapterV1 | null | undefined, language = 'English'): UseCourseStateReturn => {
    console.log('useCourseState called with:', { initialContent, chapter, language });

    // Extract data from initialContent
    const { notes, latex_code, mindmap, quiz = [], summary, og: generatedOgField, video_url, roleplay } = initialContent || {};
    ...
    const availableTabs = useMemo(() => getAvailableTabs(initialContent, chapter), [initialContent, chapter]);
