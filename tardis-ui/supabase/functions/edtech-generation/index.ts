  ...
  try {
    ...
    const openaiClient = new OpenAIClient(
      Deno.env.get('OPENAI_API_KEY') ??
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6Ijk2YWUxMjU4LTJiMDUtNDMxNi05NWJmLWMzY2RkNjk1ZDU2ZiJ9.-G-w97x4wnbsHsIOGT50cbTgfRdAvPIn3YzSclGZauo'
    )
    ...
    await Promise.all(
      types.map(async type => {
        ...
        const generated = await typeGenerators[type](
          openaiClient,
          chapter.chapter,
          language
        )
        results[type] =
          type === 'quiz' || type === 'mindmap'
            ? generated
            : generated.join('|||||')
      })
    )
