document.addEventListener("DOMContentLoaded", () => {
  ...
  //   console.log("Embed Upload Clicked !!!")
  //   fileOptions.classList.add("hidden");
  //   blackBoard.classList.remove("hidden");
  ...
  //   if (paragraphs) {
  //     console.log("Extracted Text is : ", paragraphs);
  //     try {
  //         const response = await fetch('http://localhost:5005/api/get-video-link', 
  //           {
  //             method: 'POST',
  //             headers: {
  ...
  //         // Assuming `data.videoLink` contains the path to the video file
  //         const videoLink = data.videoLink;

  //         // Step 2: Prepare the file for upload
  //         const formData = new FormData();
  //         formData.append('videoFile', await fetch(videoLink).then(res => res.blob()), 'video.mp4');

  //         // Step 3: Upload the file
  //         const uploadResponse = await fetch('http://localhost:5005/api/upload-video', {
  //             method: 'POST',
  //             body: formData
  //         });
  ...
  //     }
  //   } else {
  //       console.error('No text extracted to send.');
  //   }
  // });
  ...
  embedButton.addEventListener('click', async function() {
    if (paragraphs) {
        ...
        try {
            ...
            // const videoLink = data.videoLink;
            const videoLink = "http://localhost:8000/invideo-ai-480%20The%20Lion's%20Heartwarming%20Transformation%202024-09-26.mp4";

            if (!videoLink) {
                console.error('Video link is undefined or invalid');
                return;
            }

            // Fetch the video file as Blob
            const videoBlob = await fetch(videoLink).then(res => {
                if (!res.ok) {
